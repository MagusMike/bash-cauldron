...
def get_cmd_output(params):
    ...
    run_as_user_cmd.extend(params)
    if VERBOSE_MODE:
        return d(subprocess.check_output(run_as_user_cmd))
    else:
        with open(os.devnull, 'w') as DEVNULL:
            return d(subprocess.check_output(run_as_user_cmd, stderr=DEVNULL))
...
def get_pkcs10(key):
    ...
    base_constraints = ([crypto.X509Extension(b"keyUsage",
                                              False,
...
def get_user_agent():
    ...
    if isinstance(vernum, list) and len(vernum) > 0 and vernum[0] >= 10:
        ua = ua + ' Mac OS X '
...
def is_okta_password_exist():
    try:
        get_cmd_output(["security", "find-generic-password", "-l",
                        DEVICE_TRUST_ALIAS, "-w"])
        return True
    except Exception:
...
def verify_default_keychain():
    try:
        # default-keychain or show-keychain-info will raise an exception in cases
        # when default keychain does not exist or not in good state to accept any operations.
        keychain = get_cmd_output(['security', 'default-keychain'])
        keychain = keychain.strip().replace('"', '')
        get_cmd_output(['security', 'show-keychain-info', keychain])
        log('default keychain info: ' + keychain)
        return True
    except Exception as e:
        log_error(str(e))
        return False
...
def is_client_certificate_exist():
    try:
        ...
        expiry = datetime.datetime.strptime(
            d(certificate.get_notAfter()), '%Y%m%d%H%M%SZ')
        ...
        log_error('Failed to check if client cert exists, Error: ' + str(e))
        # Separate exception case from no certificate case
        raise e
...
def import_certificate(key, x509):
    ...
    try:
        ...
        with open(fname, 'wb') as p12_file:
        ...
        log_error('Failed to import certificate into keychain' + str(e))
        return False
    finally:
        remove_file(fname)
...
def configure_certificate():
    ...
    if is_client_certificate_exist() is True:
        return True
...
def setup_launch_agent():
    ...
    try:
        ...
        if os.path.exists(os.path.expanduser(okta_dt_launch_plist_file)):
            log('Plist File already exists, we will try to overwrite it!')

        with open(os.path.expanduser(okta_dt_launch_plist_file_tmp), 'wt') as f:
            f.write(get_launch_agent_plist())
            f.close()

        # Temp file - Following file permission operations are needed as open() is executed in root context
        subprocess.call(['chown', get_current_user(),
                         os.path.expanduser(okta_dt_launch_directory)])
        subprocess.call(['chown', get_current_user(),
                         os.path.expanduser(okta_dt_launch_plist_file_tmp)])

        # Unload old agent
        unload_and_cleanup_launch_agent()

        # Move temp file to new agent file
        subprocess.call(['mv', '-f', okta_dt_launch_plist_file_tmp, okta_dt_launch_plist_file])
...
def get_uuid():
    profile = d(subprocess.check_output(['system_profiler', 'SPHardwareDataType']))
    return profile[profile.find('Hardware UUID:'):].split()[2]


def get_display_name():
    return d(subprocess.check_output(['scutil', '--get', 'ComputerName']).strip())
...
def get_cert_auth_domain(url):
    ...
    if okta_cert_auth_domain != '':
        return okta_cert_auth_domain
    try:
        response = d(subprocess.check_output(
            [
                'curl',
                '-sS',
                '-X',
    ...
    try:
        certificate_pem = subprocess.check_output(
            [
                ...
                '-p',
                OKTA_KEY_CHAIN])
        if certificate_pem == b'' or certificate_pem == b' ':
            log('A valid Okta certificate does not exist. Skipping the renewal operation.')
            return False
        certificate = crypto.load_certificate(
            crypto.FILETYPE_PEM, certificate_pem)
        expiry = datetime.strptime(d(certificate.get_notAfter()), '%Y%m%d%H%M%SZ')
...
def get_signed_token():
    try:
        with tempfile.NamedTemporaryFile(suffix='.txt', mode='w') as token:
            with tempfile.NamedTemporaryFile(suffix='.pb7', mode='w+b') as signedToken:
        ...
        log_error('Failed to create a signed token.' + str(e))
        return False
...
def get_request(csr):
    ...
    data = dict()
    data['clientVersion'] = VERSION
    data['deviceRenewalToken'] = d(signed_token)
    data['device'] = device
    data['csr'] = d(csr)
    data['user'] = get_current_user()
    return json.dumps(data)
...
def import_certificate(key, x509):
    try:
        ...
        p12.set_privatekey(key)
        p12.set_certificate(x509)
        with tempfile.NamedTemporaryFile(suffix='.p12', mode='wb') as p12_file:
            p12_file.write(p12.export())
            p12_file.flush()
            with open(os.devnull, 'w') as DEVNULL:
...
def configure_chrome():
    log('Configuring the Chrome browser.')
    try:
        chrome_sys_configuration = d(subprocess.check_output(
    ...
    try:
        chrome_user_configuration = d(subprocess.check_output(
            ['sudo', '-u', get_current_user(),
             'defaults',
             'read',
             get_home_directory() + '/Library/Preferences/com.google.Chrome.plist',
...
def main():
    ...
    log('Running Okta Device Renewal task version : ' + VERSION)
...
def replace_text_in_launch_agent(pattern, text):
    subprocess.call(['sed',
                     ...
                     '',
                     's#' + pattern + '#' + text + '#g',
                     os.path.expanduser(okta_dt_launch_agent_tmp)])
...
def configure_device_trust_agent():
    ...
    try:
        ...
        # Create a tmp file
        with open(os.path.expanduser(okta_dt_launch_agent_tmp), 'w') as f:
            f.write(get_device_trust_launcher_code())
            f.close()

        # Temp file - Following file permission operations are needed as open() is executed in root context
        subprocess.call(
            ['chmod', '700', os.path.expanduser(okta_dt_launch_agent_tmp)])
        configure_device_trust_launcher_code()
        subprocess.call(
            ['chmod', '500', os.path.expanduser(okta_dt_launch_agent_tmp)])
        subprocess.call(['chown', get_current_user(),
                         os.path.expanduser(okta_dt_launch_agent_dir)])
        subprocess.call(['chown', get_current_user(),
                         os.path.expanduser(okta_dt_launch_agent_tmp)])

        # Move the tmp launch agent to original
        subprocess.call(['mv', '-f', okta_dt_launch_agent_tmp, okta_dt_launch_agent])
        return True
    except Exception as e:
...
def force_cleanup():
    cleanup(is_force=True)


def conditional_cleanup():
    cleanup(is_force=False)


def cleanup(is_force=False):
    is_client_cert = is_client_certificate_exist()
    is_pass = is_okta_password_exist()
    log_debug('cert exists: ' + str(is_client_cert) + ', password exists: ' + str(is_pass))
    if not (is_client_cert and is_pass) or is_force:
        log('Clean up Okta keychain, isForce: ' + str(is_force))
...
def main():
    ...
    try:
        # Verifies that the default keychain db is healthy and can accept
        # operations
        if verify_default_keychain() is False:
            log_error('Error in accessing default keychain')
...
if __name__ == "__main__":
    ...
    log('Running Okta Device Registration task version : ' + VERSION)
